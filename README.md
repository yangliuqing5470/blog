# 编程篇
## Network
- [socket网络编程知多少](./programming/network/socket.md)
- [https协议原理及使用](./programming/network/https.md)
- [nginx新手入门](./programming/network/nginx.md)

## Python
### 异步 I/O
- [协程语法](./programming/python-asyncio/asyncio-syntax.md)
- [核心概念](./programming/python-asyncio/asyncio-futures-tasks-coroutines.md)
- [调度原理](./programming/python-asyncio/asyncio-scheduling.md)
- [网络编程](./programming/python-asyncio/asyncio-networking.md)

### web 框架- aiohttp
- [HTTP 服务端框架原理](./programming/aiohttp/framework.md)
- [HTTP 客户端框架原理](./programming/aiohttp/framework_client.md)
- [WebSocket 服务端框架原理](./programming/aiohttp/websockets.md)
- [WebSocket 客户端框架原理](./programming/aiohttp/websockets_client.md)
- [性能优化及压测](./programming/aiohttp/performance_benchmarks.md)

### 并发
- [线程池](./programming/python-futures/thread-pool.md)
- [进程池](./programming/python-futures/process-pool.md)
- [Future对象](./programming/python-futures/futures.md)
- [并发编程安全及通信机制](./programming/python-futures/concurrent-safe-communication.md)

## Kubernetes
- [k8s集群搭建](./programming/k8s/cluster_create.md)
- [k8s集群网络原理](./programming/k8s/cluster_network.md)
- [私有镜像仓库搭建](./programming/k8s/images_registry.md)
- [Service详解](./programming/k8s/cluster_service.md)
- [Ingress详解](./programming/k8s/cluster_ingress.md)
- [ServiceAccount详解](./programming/k8s/cluster_serviceaccount.md)
- [工作负载详解](./programming/k8s/cluster_workloads.md)
- [持久化存储详解](./programming/k8s/cluster_storage.md)

## Docker
- [docker资源限制原理](./programming/docker/resources_limit.md)
- [docker网络知多少](./programming/docker/network.md)
- [docker资源监控](./programming/docker/resources_monitor.md)

## AWS
- [lambda原理](./programming/aws/lambda.md)

## Redis
- [redis服务框架原理](./programming/redis/framework.md)
- [redis数据类型原理](./programming/redis/data_type.md)

## gRPC
- [protobuf原理](./programming/grpc/protocol.md)
- [gRPC原理](./programming/grpc/tutorial.md)
- [gRPC编程实践](./programming/grpc/programming.md)

## ElasticSearch && Kafka
- [ElasticSearch基础介绍](./programming/elasticsearch-kafka/es_tutorial.md)

## Mysql
- [mysql索引原理](./programming/mysql/index.md)

## Windows
- [windows服务原理](./programming/windows-service/service.md)

## 问题记录
- [线上因python进程池导致的内存泄漏](./programming/issues/ProcessPollLeak.md)
- [redis模拟实现租约模式](./programming/issues/RedisKeyWithLease.md)
- [基于devpi实现的pip包镜像缓存搭建](./programming/devpi/tutorial.md)

# 工具篇
## 网络数据共享
- [http服务配置使用](./data-sharing/http/http-server.md)
- [nfs服务配置及使用](./data-sharing/nfs/nfs-server-ubuntu.md)
- [nfs问题记录](./data-sharing/nfs/nfs-issuse.md)
- [rsync服务配置及使用](./data-sharing/rsync/rsync-daemon-ubuntu.md)
- [samba服务配置及使用](./data-sharing/samba/samba-server-ubuntu.md)

# 环境配置篇
## 系统
- [ubuntu系统初始化](./environment-config/machine/ubuntu-initialization.md)

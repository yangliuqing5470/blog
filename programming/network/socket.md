...
当当前发送进行的时间片`quota`耗尽，或者其他进程需要 CPU 的时候，触发**软中断**（`NET_TX_SOFTIRQ`）发送。
...
+ 数据接收都走软中断`NET_RX`，数据发送绝大部分工作在用户进行内核态完成，
只有进行配额耗尽或者其他进行需要 CPU，才会触发软中断`NET_TX`。
...
使用场景：**用于多进行环境下，提供服务的并发能力。多个进行可用监听相同的端口**。

解决问题：**解决惊群问题，避免锁的使用**。也就是多个进行同时监听相同的端口，来了新的连接，
选择哪一个进行`accept`。`SO_REUSEPORT`在内核层面进行负载均衡，同时只有一个进行可用`accept`，
避免锁的使用。
